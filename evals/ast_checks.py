...
cli/workflowskill/loader/validator.py.
"""
...
def has_activity_named(code: str, name: str) -> bool:
    ...
    for node in ast.walk(tree):
        if _is_execute_activity_call(node):
            ...
            if node.args and isinstance(node.args[0], ast.Constant) and node.args[0].value == name:
                return True
    return False
...
def has_retry_policy_keyword(code: str, keyword: str) -> bool:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            func = node.func
            if isinstance(func, ast.Name) and func.id == "RetryPolicy" and any(
...
def activity_inside_node(code: str, node_type: type) -> bool:
    ...
    return any(isinstance(node, node_type) and _contains_activity(node) for node in ast.walk(tree))

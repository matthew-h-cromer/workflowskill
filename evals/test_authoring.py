...
async def _generate_with_retries(
    generate_skill: object, task: str, toolkit: str | None = None
) -> str:
    """Generate once (or N times if EVAL_RETRIES > 1, returning the last result)."""
    assert callable(generate_skill)
    last = await generate_skill(task, toolkit=toolkit)
...
# ---------------------------------------------------------------------------
# Test 2: Single activity — web.api
# ---------------------------------------------------------------------------


async def test_single_activity(generate_skill, parse_skill, extract_code, save_snapshot):
    """A fetch-URL workflow should call exactly one activity named 'web.api'."""
    TASK = (
        "Create a workflow named 'check-status' that takes a 'url' string input, "
        "calls the web.api action to fetch that URL, and returns the status code and content."
    )
    content = await _generate_with_retries(generate_skill, TASK, toolkit="weldable")
    save_snapshot(content)
    ...
    assert has_activity_named(code, "web.api"), (
        f"Expected activity named 'web.api'\n\nCode:\n{code}"
    )
    assert not has_explicit_timeout(code), (
        f"Simple web.api call should use default timeout, not explicit"
...
async def test_explicit_timeout(generate_skill, parse_skill, extract_code, save_snapshot):
    ...
    TASK = (
        ...
        "calls the anthropic.llm action to analyze it, and returns the result. "
    ...
    assert has_activity_named(code, "anthropic.llm"), (
        f"Expected activity named 'anthropic.llm'\n\nCode:\n{code}"
    )
...
async def test_sequential_pipeline(generate_skill, parse_skill, extract_code, save_snapshot):
    ...
    TASK = (
        ...
        "first scrapes the URL with web.scrape (extract h1 headings and article paragraphs), "
        "then summarizes the content using the anthropic.llm action, "
    ...
    assert has_activity_named(code, "web.scrape"), (
        f"Expected web.scrape activity\n\nCode:\n{code}"
    )
    assert has_activity_named(code, "anthropic.llm"), (
        f"Expected anthropic.llm activity\n\nCode:\n{code}"
    )
...
async def test_parallel_execution(generate_skill, parse_skill, extract_code, save_snapshot):
    ...
    TASK = (
        ...
        "calls both URLs concurrently using the web.api action, and returns both results. "
...
async def test_conditional_logic(generate_skill, parse_skill, extract_code, save_snapshot):
    """A workflow branching on HTTP status code should use web.api and an if statement."""
    TASK = (
        "Create a workflow named 'check-url' that takes a 'url' string input. "
        "Use the web.api action to get the response, then check the status field. "
...
async def test_loop_handling(generate_skill, parse_skill, extract_code, save_snapshot):
    ...
    TASK = (
        ...
        "Loop over each URL and call web.scrape on it. "
...
async def test_retry_policy(generate_skill, parse_skill, extract_code, save_snapshot):
    """A web.api call with explicit retry config should use RetryPolicy with maximum_attempts."""
    TASK = (
        "Create a workflow named 'retry-fetch' that takes a 'url' string input. "
        "Call the url using the web.api action with a RetryPolicy of maximum_attempts=3, "
...
async def test_error_recovery(generate_skill, parse_skill, extract_code, save_snapshot):
    """A workflow with try/except around a web.api call should have proper error handling."""
    TASK = (
        "Create a workflow named 'safe-fetch' that takes a 'url' string input. "
        "Try to call the URL using the web.api action. If an exception occurs, return "
...
async def test_llm_with_schema(generate_skill, parse_skill, extract_code, save_snapshot):
    ...
    TASK = (
        ...
        "Call the anthropic.llm action with a JSON schema constraint requiring the response "
        "to have fields 'title' (string) and 'summary' (string). Return the structured result."
    )
    content = await _generate_with_retries(generate_skill, TASK, toolkit="weldable")
    save_snapshot(content)
    ...
    assert has_schema_arg(code), (
        f"Expected 'schema' key in anthropic.llm activity args dict\n\nCode:\n{code}"
    )


# ---------------------------------------------------------------------------
# Test 11: Deterministic transform — web.scrape + Python, no anthropic.llm
# ---------------------------------------------------------------------------


async def test_deterministic_transform(generate_skill, parse_skill, extract_code, save_snapshot):
    """Counting headings is deterministic — must use web.scrape + Python, not anthropic.llm."""
    TASK = (
        "Create a workflow named 'count-headings' that takes a 'url' string input. "
        "Use web.scrape to extract all h2 headings from the page. "
        "Return a dict with 'count' (the number of headings found) and "
        "'headings' (the list of heading texts). "
        "This is pure data extraction — do not use the anthropic.llm action."
    )
    content = await _generate_with_retries(generate_skill, TASK, toolkit="weldable")
    save_snapshot(content)
    ...
    assert not has_activity_named(code, "anthropic.llm"), (
        f"Deterministic extraction should not use anthropic.llm\n\nCode:\n{code}"
    )


# ---------------------------------------------------------------------------
# Test 12: Deterministic pipeline — web.scrape + Python filter, no anthropic.llm
# ---------------------------------------------------------------------------


async def test_deterministic_pipeline(generate_skill, parse_skill, extract_code, save_snapshot):
    """Filtering links by prefix is deterministic — must use web.scrape + Python, not LLM."""
    TASK = (
        "Create a workflow named 'filter-links' that takes a 'url' string input "
        "and a 'prefix' string input (default 'https'). "
        "Use web.scrape with an object-form selector to extract href attributes from all "
        "anchor tags. Filter the list in pure Python to only include links starting with "
        "the prefix. Return the filtered list. Do not use the anthropic.llm action."
    )
    content = await _generate_with_retries(generate_skill, TASK, toolkit="weldable")
    save_snapshot(content)
    ...
    assert not has_activity_named(code, "anthropic.llm"), (
        f"Deterministic filtering should not use anthropic.llm\n\nCode:\n{code}"
    )
...
async def test_outputs_frontmatter(generate_skill, parse_skill, extract_code, save_snapshot):
    ...
    TASK = (
        ...
        "Call the anthropic.llm action to classify the sentiment as positive, negative, "
        "or neutral. Declare outputs in frontmatter for 'sentiment' (str) and "
        "'confidence' (str). Return both values."
    )
    content = await _generate_with_retries(generate_skill, TASK, toolkit="weldable")
    save_snapshot(content)
    ...
    assert has_activity_named(code, "anthropic.llm"), (
        f"Expected anthropic.llm activity for sentiment classification\n\nCode:\n{code}"
    )


# ---------------------------------------------------------------------------
# Test 14: web.scrape object-form selector
# ---------------------------------------------------------------------------
...
async def test_scrape_object_selector(generate_skill, parse_skill, extract_code, save_snapshot):
    ...
    TASK = (
        ...
        "Use web.scrape to extract all links from anchor tags — get both the link text "
        "and the href attribute using the appropriate selector forms. "
        "Return the results. Do not use the anthropic.llm action."
    )
    content = await _generate_with_retries(generate_skill, TASK, toolkit="weldable")
    save_snapshot(content)
    ...
    assert has_nested_dict_in_activity_args(code), (
        f"Expected object-form selector (nested dict) in web.scrape args\n\nCode:\n{code}"
    )
...
async def test_error_recovery_with_retry(generate_skill, parse_skill, extract_code, save_snapshot):
    """A resilient web.api call should combine RetryPolicy and try/except."""
    TASK = (
        "Create a workflow named 'resilient-fetch' that takes a 'url' string input. "
        "Call the URL with the web.api action and a RetryPolicy of maximum_attempts=3. "
...
async def test_scrape_before_llm(generate_skill, parse_skill, extract_code, save_snapshot):
    """Extracting structured job data and summarizing should use web.scrape before LLM."""
    TASK = (
        ...
        "Use web.scrape with those selectors to extract the job titles and company names. "
        "Then use the anthropic.llm action with a JSON schema to summarize the hiring "
        "landscape in 2-3 sentences. Return the summary."
    )
    content = await _generate_with_retries(generate_skill, TASK, toolkit="weldable")
    save_snapshot(content)
    ...
    assert not has_activity_named(code, "web.fetch"), (
        f"Should use web.scrape (not web.fetch) to minimize LLM input\n\nCode:\n{code}"
    )
    assert has_schema_arg(code), (
        f"Expected 'schema' key in anthropic.llm activity args dict\n\nCode:\n{code}"
    )
    assert has_scrape_feeding_llm(code), (
        f"Expected web.scrape call before anthropic.llm call in statement order\n\nCode:\n{code}"
    )
...
async def test_filter_before_llm(generate_skill, parse_skill, extract_code, save_snapshot):
    """Scraping posts and filtering to 2025 should use Python between scrape and LLM."""
    TASK = (
        "Create a workflow named 'blog-themes' that takes a 'url' string input. "
        "Use web.scrape to extract blog post titles (CSS: '.post-title')"
        ...
        "Then use the anthropic.llm action to summarize the themes of the 2025 posts. "
        "Return the summary."
    )
    content = await _generate_with_retries(generate_skill, TASK, toolkit="weldable")
    save_snapshot(content)
...
async def test_human_in_the_loop(generate_skill, parse_skill, extract_code, save_snapshot):
    ...
    TASK = (
        ...
        "Use the web.api action to submit the request to https://example.com/api/submit. "
...
async def test_signal_with_timeout(generate_skill, parse_skill, extract_code, save_snapshot):
    """A workflow with a signal deadline should use wait_for_signal with timeout."""
    TASK = (

...
def _track_eval_score(request: pytest.FixtureRequest) -> Any:
    ...
    if request.node.get_closest_marker("eval"):
        passed = not request.node.session.testsfailed
        _scores[request.node.nodeid] = request.node.rep_call.passed if hasattr(request.node, "rep_call") else True
...
_SKILL_MD_PATH = Path(__file__).parents[2] / "skill" / "SKILL.md"
